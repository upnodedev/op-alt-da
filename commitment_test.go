	...
	testCases := []tcase{
		{
			...
			expectedErr: common.ErrInvalidCommitment,
		},
		{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err == nil {
				...
				require.Equal(t, append([]byte{common.TxDataVersion1}, tc.commData...), comm.TxData())
				...
				if tc.commType != GenericCommitmentType {
					require.ErrorIs(t, common.ErrCommitmentMismatch, comm.Verify([]byte("wrong data")))
				}
			}
		})
	}
